...
export class StudentCardValidationService {
  ...
  async validate(filePath: string, inputKelas?: string, inputJurusan?: string) {
    ...
    try {
      ...
      // Crop gambar menjadi HEADLINE dan DATA
      ...
      // Preprocessing (biar teks lebih kontras & OCR lebih presisi)
      ...
      // ✅ FIXED: Split otomatis NIS & NISN
      let nisRaw = extractByLabel(['NIS/NISN', 'NIS', 'NISN']);
      let nis = '', nisn = '';

      if (nisRaw) {
        nisRaw = nisRaw.replace(/[^\d\/\s]/g, '').replace(/\s+/g, ' ').trim();
        // Jika formatnya 1022010876/0074753635 atau 1022010876 / 0074753635
        const match = nisRaw.match(/(\d{8,12})\s*[\/ ]\s*(\d{8,12})/);
        if (match) {
          nis = match[1];
          nisn = match[2];
        } else {
          // fallback: cari dua angka panjang berurutan di seluruh text
      ...
      if (!finalNama) {
        ...
        if (m) finalNama = m[1].trim();
      }

      const fieldsPresent = Boolean(finalNama && (nis || nisn) && ttlNormalized && kelasValue);
      if (!fieldsPresent && !headlineOk) {
        throw new BadRequestException('⚠️ Data kartu pelajar tidak lengkap atau teks sulit dibaca. Coba unggah ulang dengan gambar lebih jelas.');
      }

      // ===== VALIDASI DENGAN INPUT REGISTER =====
      // Ambil input kelas dan jurusan dari parameter (misal: dari body request)
      // Untuk implementasi, tambahkan parameter: validate(filePath: string, inputKelas?: string, inputJurusan?: string)
      // Di bawah ini contoh logika validasi, pastikan inputKelas dan inputJurusan dikirim dari controller
      // inputKelas dan inputJurusan langsung dari parameter

      // Normalisasi string biar perbandingan adil (tanpa spasi ganda, huruf besar/kecil disamakan)
      ...
      // Normalisasi kelas: ambil romawi saja
      ...
      // Gunakan default '' biar gak error kalau undefined
