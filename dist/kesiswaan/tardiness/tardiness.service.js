...
let TardinessService = TardinessService_1 = class TardinessService {
    ...
    constructor(recordRepo, appealRepo, summaryRepo, alertRepo, patternRepo, walasApiClient) {
    ...
    async syncTardinessFromWalas(startDate, endDate, forceSync = false) {
        try {
            this.logger.log(`Syncing tardiness from Walas (${this.formatDate(startDate)} to ${this.formatDate(endDate)})`);
            ...
            for (const record of walasData.data) {
                try {
                    ...
                    syncedCount++;
                    await this.updateMonthlySummary(record.student_id, record.class_id, this.formatDate(new Date(record.tanggal)));
                }
                catch (error) {
                    failedCount++;
                    errors.push({
                    ...
                    this.logger.error(`Failed to sync tardiness for student ${record.student_id}: ${error.message}`);
                }
            }
            this.logger.log(`Tardiness sync completed: ${syncedCount} synced, ${failedCount} failed`);
            return {
                success: true,
    ...
    isTardy(record) {
        if (record.is_late === true)
            return true;
        if (record.status_tardiness === 'yes')
            return true;
