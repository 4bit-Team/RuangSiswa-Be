...
let NotificationGateway = NotificationGateway_1 = class NotificationGateway {
    ...
    handleConnection(client) {
        ...
        if (userId) {
            ...
            this.logger.warn(`Client ${client.id} connected without userId. Cannot subscribe to notifications.`);
        }
    }
    ...
    sendNotificationToUser(userId, notification) {
        try {
            if (!this.server) {
                this.logger.warn(`WebSocket server not initialized. Notification for user ${userId} cannot be sent via WebSocket.`);
                return;
            }
            ...
            this.logger.log(`Notification sent to user ${userId} (${clientCount} active connections)`);
        }
        catch (error) {
    ...
    sendNotificationToUsers(userIds, notification) {
        try {
            if (!this.server) {
                this.logger.warn(`WebSocket server not initialized. Notifications for ${userIds.length} users cannot be sent.`);
                return;
            }
            ...
            this.logger.error(`Failed to broadcast notifications: ${error.message}`, error.stack);
        }
    }
};
